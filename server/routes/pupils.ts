...
router.get("/", requireAuth, async (req: Request, res: Response) => {
  ...
  const enriched = await Promise.all(pupils.map(async p => {
    const assignments = (await db.prepare("SELECT * FROM assignments WHERE pupil_id = ? ORDER BY assigned_at DESC").all(p.id) as any[]).map(parseAssignment);
    const attendance = await db.prepare("SELECT * FROM attendance_records WHERE pupil_id = ? ORDER BY date DESC").all(p.id);
    const behaviour = await db.prepare("SELECT * FROM behaviour_records WHERE pupil_id = ? ORDER BY date DESC LIMIT 100").all(p.id);
...
router.get("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  const assignments = (await db.prepare("SELECT * FROM assignments WHERE pupil_id = ? ORDER BY assigned_at DESC").all(pupil.id) as any[]).map(parseAssignment);
  const attendance = await db.prepare("SELECT * FROM attendance_records WHERE pupil_id = ? ORDER BY date DESC LIMIT 30").all(pupil.id);
  const behaviour = await db.prepare("SELECT * FROM behaviour_records WHERE pupil_id = ? ORDER BY date DESC LIMIT 30").all(pupil.id);
...
// ── Archive Pupil ─────────────────────────────────────────────────────────────
router.delete("/:id", requireAuth, requireMinRole("teacher"), async (req: Request, res: Response) => {
  await db.prepare("UPDATE pupils SET is_active = 0 WHERE id = ? AND school_id = ?")
...
// ── Bulk Import (CSV) ─────────────────────────────────────────────────────────
router.post("/bulk-import", requireAuth, requireMinRole("school_admin"), async (req: Request, res: Response) => {
  ...
  const insert = await db.prepare(`INSERT INTO pupils (id, school_id, name, year_group, send_need, code, upn, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
...
router.get("/safeguarding/incidents", requireAuth, requireMinRole("senco"), async (req: Request, res: Response) => {
...
router.post("/safeguarding/incidents", requireAuth, async (req: Request, res: Response) => {
  ...
  const id = uuidv4();
  await db.prepare(`INSERT INTO safeguarding_incidents (id, school_id, pupil_id, reported_by, description, ai_trigger, severity)
    VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
  ...
  if (school?.dsl_email) {
    const pupil = pupilId ? await db.prepare("SELECT name FROM pupils WHERE id = ?").get(pupilId) as any : null;
    await db.prepare("UPDATE safeguarding_incidents SET dsl_notified = 1, dsl_notified_at = NOW() WHERE id = ?").run(id);
...
router.put("/safeguarding/incidents/:id", requireAuth, requireMinRole("senco"), async (req: Request, res: Response) => {
  const { status, notes } = req.body;
  await db.prepare(`UPDATE safeguarding_incidents SET status=?, notes=?, reviewed_by=?, reviewed_at=NOW() WHERE id=? AND school_id=?`)
...
// ── Assignments ──────────────────────────────────────────────────────────────
router.post("/:id/assignments", requireAuth, async (req: Request, res: Response) => {
  ...
  const id = uuidv4();
  await db.prepare(`INSERT INTO assignments (id, pupil_id, assigned_by, title, subtitle, type, content, sections, metadata, status)
...
router.delete("/:id/assignments/:assignmentId", requireAuth, async (req: Request, res: Response) => {
  const pupil = await db.prepare("SELECT id FROM pupils WHERE id = ? AND school_id = ?").get(req.params.id, req.user!.schoolId);
  if (!pupil) return res.status(404).json({ error: "Pupil not found" });
  await db.prepare("DELETE FROM assignments WHERE id = ? AND pupil_id = ?").run(req.params.assignmentId, req.params.id);
...
router.post("/:id/attendance", requireAuth, async (req: Request, res: Response) => {
  ...
  await db.prepare(`INSERT INTO attendance_records (id, school_id, pupil_id, recorded_by, date, am_status, am_reason, pm_status, pm_reason, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(pupil_id, date) DO UPDATE SET
      ...
      notes=excluded.notes, recorded_by=excluded.recorded_by, recorded_at=NOW()`).run(
...
router.get("/:id/support-plans", requireAuth, async (req: Request, res: Response) => {
  const pupil = await db.prepare("SELECT id FROM pupils WHERE id=? AND school_id=?").get(req.params.id, req.user!.schoolId);
  if (!pupil) return res.status(404).json({ error: "Pupil not found" });
...
router.post("/:id/support-plans", requireAuth, async (req: Request, res: Response) => {
...
router.put("/:id/support-plans/:planId", requireAuth, async (req: Request, res: Response) => {
  const plan = await db.prepare("SELECT * FROM behaviour_support_plans WHERE id=? AND school_id=?").get(req.params.planId, req.user!.schoolId) as any;
  if (!plan) return res.status(404).json({ error: "Plan not found" });
  const { status, sharedWithParents } = req.body;
  await db.prepare("UPDATE behaviour_support_plans SET status=?, shared_with_parents=?, updated_at=NOW() WHERE id=?").run(
    status || plan.status,
...
router.delete("/:id/support-plans/:planId", requireAuth, async (req: Request, res: Response) => {
  const plan = await db.prepare("SELECT * FROM behaviour_support_plans WHERE id=? AND school_id=?").get(req.params.planId, req.user!.schoolId);
  if (!plan) return res.status(404).json({ error: "Plan not found" });
  await db.prepare("DELETE FROM behaviour_support_plans WHERE id=?").run(req.params.planId);
