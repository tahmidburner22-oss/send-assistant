...
CREATE INDEX IF NOT EXISTS idx_presentation_library_subject ON presentation_library(subject);
CREATE INDEX IF NOT EXISTS idx_presentation_library_curated ON presentation_library(curated);
...
CREATE TABLE IF NOT EXISTS diagram_library (
  ...
  source TEXT NOT NULL DEFAULT 'ai',
  curated INTEGER NOT NULL DEFAULT 0,
  diagram_type TEXT NOT NULL DEFAULT 'diagram_a',
  stage TEXT,
  discipline TEXT,
  subtopic TEXT NOT NULL DEFAULT '',
  canonical_topic_key TEXT,
  canonical_subtopic_key TEXT,
  visual_profile TEXT,
  specification_refs TEXT NOT NULL DEFAULT '[]',
  required_practical INTEGER NOT NULL DEFAULT 0,
  generator_model TEXT,
  generation_prompt TEXT,
  prompt_version TEXT,
  review_status TEXT NOT NULL DEFAULT 'legacy',
  scientific_review_notes TEXT,
  alt_text TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
export async function initDb() {
  ...
  const alterMigrations = [
    ...
    `CREATE INDEX IF NOT EXISTS idx_assets_library_entry ON worksheet_library_assets(library_entry_id)`,
    `CREATE INDEX IF NOT EXISTS idx_assets_section_key ON worksheet_library_assets(section_key)`,
    // Diagram-library classification, provenance, scientific quality, and accessibility.
    ...
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN stage TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN discipline TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN subtopic TEXT NOT NULL DEFAULT ''; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN canonical_topic_key TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN canonical_subtopic_key TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN visual_profile TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN specification_refs TEXT NOT NULL DEFAULT '[]'; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN required_practical INTEGER NOT NULL DEFAULT 0; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN generator_model TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN generation_prompt TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN prompt_version TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN review_status TEXT NOT NULL DEFAULT 'legacy'; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN scientific_review_notes TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE diagram_library ADD COLUMN alt_text TEXT; EXCEPTION WHEN duplicate_column THEN NULL; END $$`,
    `CREATE INDEX IF NOT EXISTS idx_diagram_library_type ON diagram_library(diagram_type)`,
    `CREATE INDEX IF NOT EXISTS idx_diagram_library_science_lookup ON diagram_library(stage, discipline, canonical_topic_key, canonical_subtopic_key, diagram_type)`,
    `CREATE INDEX IF NOT EXISTS idx_diagram_library_review ON diagram_library(review_status, generator_model)`,
