...
- **PR-1 .. PR-4** = the work units. Each maps to one branch and one
  pull request, sized so a fresh chat can finish it without
  exhausting context.
...
```
Context: send-assistant repo. Branch <see SESSION-HANDOFF.md "What is
         in flight"> off main. Working on PR-1 of the four-PR ladder
...
post-validator output. That gives us a regression gate. It does NOT
yet give us a **quality signal** — the rules check that warnings
weren't raised, but they don't grade the worksheet's actual content
on the axes a teacher cares about (curriculum fidelity, command-word
discipline, scaffolding, SEND register, examiner voice).
...
7. **Sprint 7 — Public scorecard.** Surface the eval-report
   markdown on a public status page so partners can see the trend.
...
Within PR-1, work proceeds in alphabetised sub-steps so each is one
checkpoint-able commit. **Push after every sub-step.**

### Sprint 1 — Measurement

| Sub-step | Title                                            | Touches                                                      |
| -------- | ------------------------------------------------ | ------------------------------------------------------------ |
| 1.A      | Per-axis rubric                                  | `server/tests/worksheet-eval/rules.ts` (extend registry)     |
...
- 1.C → 1.D: schema must accept rater output before the rater can
  stamp into it.
- 1.D → 1.E: rater must be callable before the runner can invoke it.
- 1.E → 1.F: runner must produce reports before a baseline can be
  frozen.
- 3.A → 3.B: orchestrator must exist before the prefix-stripper has
  a caller.
...
- Do not change the seven built-in rule names (`mcq-single-correct`,
  `word-bank-deduped`, `no-foreign-diagrams`, `reading-age-in-range`,
  `spec-ref-present`, `send-fidelity-floor`, `qa-score-floor`). New
  rules are additive.
- Comparison-corpus fixtures use the `cmp-` id prefix so they don't
  collide with the original 50 fixtures.
...
- Sandbox is INTEGRATIONS_ONLY — never run `npm install`. Type-check
  + tests run in CI on PR push.
...
server/tests/worksheet-eval/modelJudgeRater.ts                 (new)
server/tests/worksheet-eval/humanScoresLoader.ts               (new)
server/tests/worksheet-eval/fixtures/cmp-*.json                (new — 30 entries)
server/tests/worksheet-eval/fixtures/comparison-corpus-human-scores.csv (new)
server/tests/worksheet-eval/baseline.json                      (new)
client/src/lib/twoPassWorksheetOrchestrator.ts                 (new)
server/tests/worksheet-eval-rules-axis-floor.test.ts           (new)
server/tests/worksheet-eval-summariser.test.ts                 (new)
server/tests/worksheet-eval-rater.test.ts                      (new)
server/tests/worksheet-eval-runner-human-scores.test.ts        (new)
server/tests/worksheet-eval-experiments.test.ts                (new)
server/tests/twoPassWorksheetOrchestrator.test.ts              (new)
server/tests/perSubjectPromptFamilies.test.ts                  (new)
.github/workflows/worksheet-eval.yml                           (extend — baseline refresh job)
...
- [ ] All six Sprint 1 sub-steps shipped + all four Sprint 3
      sub-steps shipped.
- [ ] `SESSION-HANDOFF.md` "What is done" lists every sub-step with
      its commit sha; "What is in flight" is empty; "What is next"
      points at PR-3 (next dependent PR — PR-2 lands in parallel).
...
- **Single source of truth.** Rubric lives in one file; the rater
  and the summariser both import from it.
...
- **Idempotent / pure validators.** Running the rater twice yields
  the same axis scores.
- **Conservative.** When in doubt, the rater warns (it doesn't
  rewrite the worksheet — that's the orchestrator's job).
- **Push, don't accumulate.** After every sub-step, push to remote.
  Never hold > one logical chunk locally (rule #1 of phase-e RESUME).
