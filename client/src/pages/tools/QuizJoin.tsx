...
export default function QuizJoin() {
  ...
  // SEND Accessibility settings
  const [showA11yPanel, setShowA11yPanel] = useState(false);
  const [fontSize, setFontSize]     = useState<"normal" | "large" | "xlarge">("normal");
  const [dyslexiaFont, setDyslexiaFont] = useState(false);
  const [colourOverlay, setColourOverlay] = useState<string | null>(null);
  ...
  // Derived styles from accessibility settings
  const a11yQFontSize   = fontSize === "xlarge" ? "text-3xl" : fontSize === "large" ? "text-2xl" : "text-xl";
  const a11yOptFontSize = fontSize === "xlarge" ? "text-xl"  : fontSize === "large" ? "text-lg"  : "text-base";
  const a11yFontFamily  = dyslexiaFont ? { fontFamily: "'Comic Sans MS', 'OpenDyslexic', 'Arial', sans-serif" } : {};
  const overlayStyle    = colourOverlay ? { backgroundColor: colourOverlay, mixBlendMode: "multiply" as const } : {};

  const OVERLAYS = [
    { label: "None",       value: null },
    { label: "Cream",      value: "rgba(255,248,220,0.55)" },
    { label: "Blue",       value: "rgba(200,220,255,0.45)" },
    { label: "Mint",       value: "rgba(200,245,220,0.45)" },
    { label: "Lavender",   value: "rgba(230,210,255,0.45)" },
  ];
      ...
      <AnimatePresence>
        {showA11yPanel && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className="absolute top-14 right-4 z-50 bg-white rounded-2xl shadow-2xl p-4 w-72 border border-indigo-100">
            <p className="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
              <Settings className="w-4 h-4 text-indigo-500" /> Accessibility Settings
            </p>
            <div className="space-y-3">
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-1.5 flex items-center gap-1"><Type className="w-3 h-3" /> Text Size</p>
                <div className="flex gap-2">
                  {(["normal","large","xlarge"] as const).map(s => (
                    <button key={s} onClick={() => setFontSize(s)}
                      className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${fontSize === s ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-400"}`}>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-1.5">Dyslexia Font</p>
                <button onClick={() => setDyslexiaFont(f => !f)}
                  className={`w-full py-1.5 rounded-lg text-xs font-semibold border transition-colors ${dyslexiaFont ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-400"}`}>
                  {dyslexiaFont ? "On — Comic/Dyslexia font" : "Off — Standard font"}
                </button>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-1.5 flex items-center gap-1"><Eye className="w-3 h-3" /> Colour Overlay</p>
                <div className="flex gap-1.5 flex-wrap">
                  {OVERLAYS.map(o => (
                    <button key={o.label} onClick={() => setColourOverlay(o.value)}
                      className={`px-2 py-1 rounded-md text-xs font-semibold border transition-colors ${colourOverlay === o.value ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-400"}`}>
                      {o.label}
                    </button>
                  ))}
                </div>
  ...
  if (room?.phase === "question" && room.currentQ) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex flex-col relative"
           style={colourOverlay ? { position: "relative" } : {}}>
        ...
        <div className="px-4 pt-4 pb-2 relative z-20">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowA11yPanel(p => !p)}
        ...
        {/* Question */}
        <div className="flex-1 flex flex-col px-4 relative z-20">
          <AnimatePresence mode="wait">
            <motion.div key={room.currentQuestion} initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
              ...
              style={a11yFontFamily}>
              <p className={`text-white ${a11yQFontSize} font-bold leading-snug`} style={a11yFontFamily}>{q.question}</p>
            </motion.div>
          </AnimatePresence>
            ...
            <div className="grid grid-cols-2 gap-3 mt-auto mb-6">
              {q.options.map((opt, i) => (
                <motion.button key={i} whileTap={{ scale: 0.95 }}
                  ...
                  className={`${ANSWER_COLOURS[i]} rounded-2xl p-4 text-white font-bold flex flex-col items-center gap-2 shadow-lg min-h-[80px] justify-center`}
                  style={a11yFontFamily}>
                  <span className="text-2xl">{ANSWER_SHAPES[i]}</span>
                  <span className={`text-center leading-tight ${a11yOptFontSize}`}>{opt}</span>
                </motion.button>
              ))}
            </div>
          ) : (
