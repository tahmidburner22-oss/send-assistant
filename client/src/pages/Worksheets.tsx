...
REMEMBER: Every question must be COMPLETE, CORRECT, and SPECIFIC to the topic. Do not generate placeholder text or incomplete questions.`;
  ...
  const handleGenerate = async () => {
    ...
    if (generatedWs) {
      ...
      try {
        ...
        const pipelineResult = runWorksheetPipeline(
          {
            ...
            subject,
            yearGroup,
            phase: (yearGroup.toLowerCase().includes('year 1') || yearGroup.toLowerCase().includes('year 2') ||
                    yearGroup.toLowerCase().includes('year 3') || yearGroup.toLowerCase().includes('year 4') ||
                    yearGroup.toLowerCase().includes('year 5') || yearGroup.toLowerCase().includes('year 6') ||
                    yearGroup.toLowerCase().includes('ks1') || yearGroup.toLowerCase().includes('ks2'))
                   ? 'primary' : 'secondary',
        ...
        if (pipelineResult.validationReport.warnings.length > 0) {
          console.warn('[Pipeline] Warnings:', pipelineResult.validationReport.warnings);
        }
        if (!pipelineResult.readyToRender) {
          console.error('[Pipeline] Validation failed:', pipelineResult.validationReport.errors);
        }
        // Attach auto-generated diagram SVGs to metadata
        if (Object.keys(pipelineResult.diagramSVGs).length > 0) {
          (generatedWs.metadata as any).autoGeneratedDiagrams = pipelineResult.diagramSVGs;
        }
      } catch (pipelineErr) {
  ...
  // ─── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="teacher-studio-bg min-h-[calc(100vh-4rem)] px-4 py-5 sm:px-6 sm:py-7">
      <div className="max-w-7xl mx-auto space-y-5">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      </motion.div>

      {!generated && (
        <section className="teacher-studio-surface overflow-hidden rounded-[1.35rem] p-5 sm:p-7" aria-labelledby="worksheet-studio-title">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_290px] lg:items-end">
            <div>
              <div className="flex items-center gap-2 text-brand teacher-studio-eyebrow">
                <span className="inline-flex h-5 w-5 items-center justify-center rounded-md bg-brand text-[10px] text-white">A</span>
                Teaching workspace
              </div>
              <h2 id="worksheet-studio-title" className="mt-3 max-w-2xl text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Plan a worksheet with confidence.</h2>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">Move from curriculum intent to a professionally formatted, SEND-aware resource without losing control of the learning goal or protected print layout.</p>
            </div>
            <div className="grid grid-cols-3 gap-2 rounded-2xl border border-border/60 bg-muted/35 p-2.5" aria-label="Worksheet workflow">
              <div className="rounded-xl bg-white px-2.5 py-3 shadow-sm ring-1 ring-border/50">
                <span className="text-[10px] font-bold text-brand">01</span>
                <p className="mt-1 text-xs font-semibold text-foreground">Plan</p>
                <p className="mt-0.5 text-[10px] leading-tight text-muted-foreground">Curriculum focus</p>
              </div>
              <div className="rounded-xl px-2.5 py-3">
                <span className="text-[10px] font-bold text-muted-foreground">02</span>
                <p className="mt-1 text-xs font-semibold text-foreground">Build</p>
                <p className="mt-0.5 text-[10px] leading-tight text-muted-foreground">SEND-aware output</p>
              </div>
              <div className="rounded-xl px-2.5 py-3">
                <span className="text-[10px] font-bold text-muted-foreground">03</span>
                <p className="mt-1 text-xs font-semibold text-foreground">Teach</p>
                <p className="mt-0.5 text-[10px] leading-tight text-muted-foreground">Print or assign</p>
              </div>
            </div>
          </div>
        </section>
      ...
      {!generated ? (
        <Tabs value={activeTab} onValueChange={(tab) => {
          ...
          <TabsList className="teacher-studio-surface flex h-auto w-max min-w-full gap-1 rounded-2xl p-1.5">
            <TabsTrigger value="generate" className="rounded-xl text-xs gap-1 flex-1 min-w-[80px] data-[state=active]:bg-brand data-[state=active]:text-white data-[state=active]:shadow-sm"><Sparkles className="w-3 h-3" /><span className="hidden xs:inline">Generate</span><span className="xs:hidden">Gen</span></TabsTrigger>
            <TabsTrigger value="upload" className="rounded-xl text-xs gap-1 flex-1 min-w-[72px] data-[state=active]:bg-brand data-[state=active]:text-white data-[state=active]:shadow-sm"><Upload className="w-3 h-3" /> Upload</TabsTrigger>
            {showLibraryTab && (
              <TabsTrigger value="bank" className="rounded-xl text-xs gap-1 flex-1 min-w-[72px] data-[state=active]:bg-brand data-[state=active]:text-white data-[state=active]:shadow-sm"><Library className="w-3 h-3" /> Library</TabsTrigger>
            )}
            <TabsTrigger value="exam-questions" className="rounded-xl text-xs gap-1 flex-1 min-w-[80px] data-[state=active]:bg-brand data-[state=active]:text-white data-[state=active]:shadow-sm"><Award className="w-3 h-3" /><span className="hidden sm:inline">Exam Bank</span><span className="sm:hidden">Exams</span></TabsTrigger>
            <TabsTrigger value="history" className="rounded-xl text-xs gap-1 flex-1 min-w-[72px] data-[state=active]:bg-brand data-[state=active]:text-white data-[state=active]:shadow-sm">
          ...
          <TabsContent value="generate" className="mt-4">
            <Card className="teacher-studio-surface overflow-hidden rounded-[1.35rem] border-0 shadow-none">
              <CardContent className="p-4 space-y-6 sm:p-6 lg:p-7">
                ...
                <Tabs value={generationMode} onValueChange={(v) => setGenerationMode(v === "auto-class" ? "auto-class" : "manual")}>
                  <TabsList className="grid h-11 w-full max-w-xl grid-cols-2 rounded-2xl border border-border/60 bg-muted/40 p-1 mx-auto">
                    <TabsTrigger value="manual" className="rounded-xl text-xs gap-1 data-[state=active]:bg-white data-[state=active]:shadow-sm" data-testid="ws-mode-manual">
                      <PenLine className="w-3 h-3" /> Manual
                    </TabsTrigger>
                    <TabsTrigger value="auto-class" className="rounded-xl text-xs gap-1 data-[state=active]:bg-white data-[state=active]:shadow-sm" data-testid="ws-mode-auto-class">
                ...
                <div className={generationMode === "auto-class" ? "hidden" : "space-y-6"} data-testid="ws-manual-form">

                {/* Page heading */}
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <p className="teacher-studio-eyebrow text-brand">Guided builder</p>
                    <h2 className="mt-1 text-xl font-bold tracking-tight text-foreground">Start with the lesson, then tune the support.</h2>
                  </div>
                  <p className="max-w-sm text-xs leading-relaxed text-muted-foreground">Use the quick plan for speed, or choose every setting below. Your saved worksheet history is always retained separately.</p>
                </div>

                {/* Natural Language Input */}
                <div className="relative overflow-hidden rounded-2xl border border-brand/20 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-4 sm:p-5 space-y-3 shadow-sm">
                  <Label className="text-sm font-semibold text-foreground flex items-center gap-1.5"><span className="inline-flex h-6 w-6 items-center justify-center rounded-lg bg-brand text-white"><Wand2 className="h-3.5 w-3.5" /></span> Describe the lesson you need</Label>
                  <div className="flex gap-2 items-start">
                    {nlExpanded ? (
                      <Textarea
                        ...
                        className="min-h-[100px] flex-1 resize-none rounded-xl border-border/70 bg-white text-sm shadow-sm"
                        autoFocus
                        rows={4}
                      />
                    ) : (
                      <Input
                        ...
                        className="h-11 flex-1 rounded-xl border-border/70 bg-white shadow-sm"
                      />
                    )}
                    <Button onClick={handleNlInput} disabled={!nlInput.trim()} size="sm" className="h-11 shrink-0 rounded-xl bg-brand px-5 text-white shadow-sm hover:bg-brand/90">
                ...
                {/* AI Toggle */}
                <div className="flex items-center justify-between rounded-2xl border border-slate-800/10 bg-slate-950 p-4 text-white shadow-sm">
                  <div>
                    <p className="text-sm font-semibold text-white">AI-assisted resource design</p>
                    <p className="mt-0.5 text-xs text-slate-300">Curriculum-aligned content with provider resilience in the background</p>
                  </div>
                  <Switch checked={useAI} onCheckedChange={setUseAI} />
                </div>

                {/* Core fields */}
                <div className="rounded-2xl border border-border/60 bg-white/70 p-4 sm:p-5 space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold text-foreground flex items-center gap-2"><span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-brand-light text-brand"><FileText className="h-3.5 w-3.5" /></span> Lesson essentials</h3><span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Step 1 of 2</span></div>
                ...
                <div className="no-print sticky bottom-3 z-20 rounded-2xl border border-slate-900/10 bg-white/95 p-3 shadow-[0_18px_46px_-26px_rgba(15,23,42,0.42)] backdrop-blur-xl sm:p-4">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                      <p className="teacher-studio-eyebrow text-muted-foreground">Ready to build</p>
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        <span className={`rounded-md px-2 py-1 text-xs font-semibold ${subject ? "bg-brand-light text-brand" : "bg-muted text-muted-foreground"}`}>{subjects.find(s => s.id === subject)?.name || "Choose subject"}</span>
                        <span className={`rounded-md px-2 py-1 text-xs font-semibold ${yearGroup ? "bg-slate-100 text-slate-700" : "bg-muted text-muted-foreground"}`}>{yearGroup || "Choose year"}</span>
                        <span className={`max-w-[240px] truncate rounded-md px-2 py-1 text-xs font-semibold ${topic ? "bg-amber-50 text-amber-800" : "bg-muted text-muted-foreground"}`}>{topic || "Choose topic"}</span>
                        {sendNeed && sendNeed !== "none-selected" && <span className="rounded-md bg-indigo-50 px-2 py-1 text-xs font-semibold text-indigo-700">{sendNeeds.find(n => n.id === sendNeed)?.name || "SEND support"}</span>}
                      </div>
                      {usesApprovedMathsTemplates && <p className="mt-2 flex items-center gap-1.5 text-[10px] font-medium text-emerald-800"><Lock className="h-3 w-3" /> Format protected: exactly two A4 landscape pages; white paper and fixed geometry.</p>}
                    </div>
                    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
                      <Button
                        ...
                        className="h-11 rounded-xl border-brand/30 px-4 text-brand hover:bg-brand-light/30 gap-2"
                      >
                        {diagnosticLoading
                          ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" />Generating Diagnostic...</>
                          : <><ClipboardCheck className="w-3.5 h-3.5" />Diagnostic</>}
                      </Button>
                      <Button onClick={handleGenerate} disabled={loading} className="h-11 rounded-xl bg-brand px-5 text-base font-semibold text-white shadow-sm hover:bg-brand/90">
                        {loading
                          ? <><RefreshCw className="w-4 h-4 mr-2 animate-spin" />{generationStatus || (useAI ? "Building resource..." : "Generating...")}</>
                          : <><Sparkles className="w-4 h-4 mr-2" /> Build worksheet</>}
                      </Button>
                    </div>
                  </div>
                </div>
