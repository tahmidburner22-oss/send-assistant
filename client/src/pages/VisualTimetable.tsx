...
export default function VisualTimetable() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Build a standalone HTML document for the timetable
      const fontSizePx = fontSize === "small" ? 11 : fontSize === "large" ? 16 : 13;
      let rows = "";
      const daysToRender = viewMode === "day" ? schedule.filter(d => d.day === selectedDay) : schedule;

      for (const day of daysToRender) {
        let cells = "";
        if (day.slots.length === 0) {
          cells = `<div style="color:#999;font-size:11px;text-align:center;padding:12px">No activities</div>`;
        } else {
          for (const slot of day.slots) {
            const iconData = ACTIVITY_ICONS[slot.icon] || ACTIVITY_ICONS.custom;
            cells += `<div style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-left:3px solid ${slot.color};border-radius:6px;margin-bottom:4px;background:#fafafa">`;
            if (symbolStyle === "emoji") {
              cells += `<span style="font-size:20px">${slot.emoji}</span>`;
            } else {
              cells += `<span style="display:inline-block;width:24px;height:24px;border-radius:4px;background:${slot.color}20"></span>`;
            }
            cells += `<div><div style="font-weight:600;font-size:${fontSizePx}px">${slot.activity}</div>`;
            if (slot.time || slot.duration) {
              cells += `<div style="font-size:10px;color:#888">${slot.time || ""}${slot.time && slot.duration ? " · " : ""}${slot.duration ? slot.duration + " min" : ""}</div>`;
            }
            if (slot.notes) cells += `<div style="font-size:10px;color:#888;font-style:italic">${slot.notes}</div>`;
            cells += `</div></div>`;
          }
        }
        rows += `<div style="flex:1;min-width:140px;border:1px solid #e5e7eb;border-radius:8px;padding:8px"><h3 style="font-weight:700;font-size:14px;margin:0 0 8px;text-align:center;color:#111">${day.day}</h3>${cells}</div>`;
      }

      const title = `Visual Timetable${studentName ? " — " + studentName : ""}`;
      const weekLabel = weekOf ? `Week beginning: ${new Date(weekOf).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}` : "";

      // Render HTML into a hidden div, capture with html2canvas, save with jsPDF
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{font-family:system-ui,-apple-system,sans-serif;margin:24px;color:#111;-webkit-print-color-adjust:exact;print-color-adjust:exact}h1{text-align:center;font-size:22px;margin:0 0 4px}p.sub{text-align:center;font-size:13px;color:#666;margin:0 0 16px}.grid{display:flex;gap:10px;flex-wrap:wrap}</style></head><body><h1>${title}</h1>${weekLabel ? `<p class="sub">${weekLabel}</p>` : ""}<div class="grid">${rows}</div><p style="text-align:center;font-size:10px;color:#bbb;margin-top:20px">Generated by Adaptly</p></body></html>`;

      // Render into a hidden same-origin container for html2canvas capture
      const container = document.createElement("div");
      container.style.cssText = "position:fixed;top:0;left:0;width:1100px;opacity:0;pointer-events:none;z-index:99999;background:#fff";
      ...
      try {
        ...
        const safeName = (studentName || "Timetable").replace(/[^a-zA-Z0-9\s]/g, "").replace(/\s+/g, "_");
        pdf.save(`Visual_Timetable_${safeName}.pdf`);
