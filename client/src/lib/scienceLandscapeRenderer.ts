...
/** Reuses the existing fixed-height purpose box; no worksheet geometry is added or moved. */
function purposeContent(options: ScienceLandscapeOptions, fallback: string): string {
  const objective = options.learningObjective?.trim();
  return objective ? `<b>Learning objective:</b> ${esc(objective)}` : fallback;
}
