...
SECTION 2 — UNDERSTANDING (Q4–Q6):
  ...
  Q6 — SEQUENCING/STRUCTURED RESPONSE [4 marks]: ${isSTEM ? 'Present a formula triangle or method scaffold. Ask students to: (a) Rearrange the formula for a specified variable [1 mark] (b) Substitute given values and calculate [2 marks] (c) State the scientific law, unit or principle used [1 mark]' : 'Provide 6 events/plot points/key moments from the topic in a scrambled order. Ask students to number boxes 1–6 in the correct chronological or logical sequence. [3 marks: all correct = 3, 4–5 correct = 2, 2–3 correct = 1]'}
...
Return EXACTLY this JSON (raw JSON only):
{
  ...
  "sections": [
    ...
    {"title": "Q4 — Visual/Diagram Activity", "type": "q-label-diagram", "content": "${isSTEM ? 'Label the diagram below. Write the correct component name next to each number. [5 marks]\n[[DIAGRAM:{"type":"circuit","layout":"series","labels":[{"text":"Battery","x":10,"y":50},{"text":"Switch","x":50,"y":10},{"text":"Bulb","x":90,"y":50},{"text":"Resistor","x":50,"y":90},{"text":"Ammeter","x":30,"y":30}]}]]' : 'Complete the spider diagram. Add ONE descriptive word on each spoke. [5 marks]\n[ASCII spider diagram with central oval: ${params.topic}\n    ___\n   /   \\\n  | ${params.topic} |\n   \\___/\n   / | \ \\\n  /  |  \\  \\\n___ ___ ___ ___\nSpoke 1: _____ Spoke 2: _____ Spoke 3: _____ Spoke 4: _____ Spoke 5: _____]'}", "marks": 5},
    ...
    {"title": "Q6 — Sequencing/Structured Response", "type": "q-data-table", "content": "${isSTEM ? 'Use the formula triangle to answer the questions below. [4 marks]\n(a) Rearrange the formula for [variable]. [1 mark]\n(b) Calculate [quantity] given: [values]. Show all working. [2 marks]\n(c) State the scientific law or unit used. [1 mark]' : 'Number the events 1–6 in the correct order. [3 marks]\n[ ] [event/plot point 1 from ${params.topic}]\n[ ] [event/plot point 2 from ${params.topic}]\n[ ] [event/plot point 3 from ${params.topic}]\n[ ] [event/plot point 4 from ${params.topic}]\n[ ] [event/plot point 5 from ${params.topic}]\n[ ] [event/plot point 6 from ${params.topic}]\n3 marks: all correct | 2 marks: 4–5 correct | 1 mark: 2–3 correct'}", "marks": 4},
  ...
  // Strict 5-second timeout so a slow Wikimedia response never delays the worksheet.
  if (params.generateDiagram) {
    try {
      const diagramSection = await Promise.race([
        aiGenerateWorksheetDiagram({
          subject: params.subject,
          topic: params.topic,
      ...
      if (diagramSection) {
        ...
        console.info('[Diagram] Timed out or unavailable — worksheet shown without diagram');
      }
    } catch (err) {
