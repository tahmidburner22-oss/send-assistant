/**
 * build-base.mjs — render worksheet.base.json to the BASE (canonical) PDF.
 *
 * This is the non-SEND, library-canonical edition of the multi-page portrait
 * worksheet. It is a faithful mirror of build.mjs: the document STRUCTURE and
 * LAYOUT are byte-for-byte the same (intro page sections, one question per
 * page, x/y answer lines bottom-right, repeated method-reminder strip,
 * "how to start" frames on Q1–Q2, self-reflection page, teacher key page,
 * difficulty pips, marks). Only the SEND adaptations are removed:
 *
 *   - Cream #FFF8E7 full-bleed overlay        -> plain white page
 *   - OpenDyslexic typeface                   -> standard sans-serif stack
 *   - line-height 1.7 / letter 0.04em /       -> normal typographic defaults
 *     word-spacing 0.16em
 *   - Purple #5b3fa8 region borders + accent  -> neutral ink/grey
 *   - "SEND adaptations made" key section     -> removed
 *
 * The generator layers the SEND overlay, difficulty (foundation/mixed/higher)
 * and reading age back on top of this base WITHOUT changing the structure.
 */
...
const ws = JSON.parse(readFileSync(join(__dirname, "worksheet.base.json"), "utf8"));

// Base (canonical) palette — neutral, no SEND overlay.
const PAGE_BG = "#ffffff";                 // plain white page (no cream overlay)
const INK = "#111111";                     // body text
const BORDER = "#333333";                  // neutral region borders (no colour coding)
const HEAD = "#111111";                    // headings in ink, not accent colour
const FONT = "Arial, 'Helvetica Neue', Helvetica, sans-serif"; // standard typeface
...
writeFileSync(join(__dirname, "worksheet.base.html"), html, "utf8");

const outPdf = join(__dirname, "Quadratic-Simultaneous-Equations-Base.pdf");
