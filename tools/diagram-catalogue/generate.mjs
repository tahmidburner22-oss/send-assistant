...
// ── Write a phone-friendly plain-text companion ──────────────────────────────
// Same data as the CSV, but grouped Phase → Subject → Year → Topic for easy
// scanning on a phone, in iOS Files / Notes / any plain-text viewer. Useful
// for forwarding the to-do list to an artist or a non-technical reviewer.
const txtLines = [];
txtLines.push("Diagram Library — Catalogue");
txtLines.push("=".repeat(60));
txtLines.push("");
txtLines.push(
  `${ctx.count.toLocaleString()} unique diagram briefs spanning Reception → A-Level.`,
);
txtLines.push("");
txtLines.push("Generated by tools/diagram-catalogue/generate.mjs.");
txtLines.push("Do not edit by hand — re-run the generator instead.");
txtLines.push("");

const phaseOrder = [
  ["Primary (Y1–Y6)", (b) => !["KS3", "GCSE", "A-Level"].includes(b)],
  ["KS3 (Y7–Y9)", (b) => b === "KS3"],
  ["GCSE (Y10–Y11)", (b) => b === "GCSE"],
  ["A-Level (Y12–Y13)", (b) => b === "A-Level"],
];

for (const [phaseName, predicate] of phaseOrder) {
  const phaseRows = ctx.rows.filter((r) => predicate(r.year_band));
  if (!phaseRows.length) continue;

  txtLines.push("");
  txtLines.push("=".repeat(60));
  txtLines.push(`${phaseName}  —  ${phaseRows.length.toLocaleString()} diagrams`);
  txtLines.push("=".repeat(60));

  // Group by subject (preserve the order the modules added them)
  const seenSubjectOrder = [];
  const bySubject = new Map();
  for (const r of phaseRows) {
    if (!bySubject.has(r.subject)) {
      bySubject.set(r.subject, []);
      seenSubjectOrder.push(r.subject);
    }
    bySubject.get(r.subject).push(r);
  }

  for (const subject of seenSubjectOrder) {
    const subjectRows = bySubject.get(subject);
    txtLines.push("");
    txtLines.push(`# ${subject}  (${subjectRows.length} diagrams)`);
    txtLines.push("-".repeat(60));

    // Group by year_group → topic so the headings are scannable
    const seenYearOrder = [];
    const byYear = new Map();
    for (const r of subjectRows) {
      if (!byYear.has(r.year_group)) {
        byYear.set(r.year_group, []);
        seenYearOrder.push(r.year_group);
      }
      byYear.get(r.year_group).push(r);
    }

    for (const year of seenYearOrder) {
      const yearRows = byYear.get(year);
      const seenTopicOrder = [];
      const byTopic = new Map();
      for (const r of yearRows) {
        if (!byTopic.has(r.topic)) {
          byTopic.set(r.topic, []);
          seenTopicOrder.push(r.topic);
        }
        byTopic.get(r.topic).push(r);
      }
      for (const topic of seenTopicOrder) {
        txtLines.push("");
        txtLines.push(`## ${year}  ·  ${topic}`);
        for (const r of byTopic.get(topic)) {
          txtLines.push(`  [${r.id}]  ${r.title}`);
        }
      }
    }
  }
...
const txtPath = path.join(repoRoot, "docs", "diagram-library-catalogue.txt");
