...
> **Update — diagram catalogue is now Reception → A-Level.** The original 2,052
> primary briefs in `docs/diagram-library-catalogue.csv` have been joined by ~3,920
> new KS3, GCSE and A-Level briefs (generated by `tools/diagram-catalogue/`).
> Workstreams W3 / W4 / W6 still apply to primary, but the catalogue itself is no
> longer primary-only — it covers every UK year band a teacher might ask Adaptly
> to make a worksheet for.
